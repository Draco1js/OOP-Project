...
public:
    ...
    static sf::IntRect update(const std::string &animation, sf::Sprite &sprite);
    ...
    // Function to check if an animation has finished playing
    static bool isAnimationFinished(const std::string &animation);

    // Function to clean up all animations and resources
    static void cleanup();
};
